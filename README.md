...
> [!IMPORTANT]
> 🛡️ **Trust Center Settings:** Before using this add-in, you must configure your Excel Trust Center settings in your system to allow the add-in to work. *(Just a One-Time Setup)*. [👉 Click here to view the step-by-step Setup guide📖](https://github.com/MRF-UTILITY/MRF/raw/main/Guide%20to%20Install%20Add-in%20file%20in%20Excel.pptx "View the Step-by-step Trust Center Settings Guide").
...
6. **Browse and Install:** Click **Browse**, locate your downloaded `.xlam` add-in file, select it, and click **OK**.
7. **Access the Utility:** You will now see a dedicated **M R F** tab in your Excel Ribbon!

> [!NOTE]
> **Troubleshooting:** If you have followed all the above Installation steps properly and still the **M R F** ribbon tab does not appear, simply restart Excel.
...
3. **Execute Workflows:**
   ...
   * **⚡Load Factor Dashboard Tool:** Run to instantly generate comprehensive pivot dashboards, Route Analysis, and Trip-wise Summaries. **Important:** Export data using T-code (`ZGFN_FREIGHT`) and ensure you select the proper layout with Subtotals containing the required columns before exporting to spreadsheet.
...
> [!WARNING]
> ⚠️ ***Very Important Pull Compliance Step | Only for FDC to SOF Pull Purpose :*** Users must manually review & update the given `Source File.xlsx` workbook located in `D:\FDC TO SOF\` themselves for any missing/Incorrect material codes where correct INS Tube/Flaps are not appearing. You must also add any new Tube-Type Tyre sizes to the list from time to time to ensure proper set billing from FDC to SOF's in IBTAs. Keeping this file up to date is essential for accurate set billing to SOFs.

---

## 🔄 Updates & Version Control

<span style="color:#0078D4;">This add-in will automatically check against the GitHub repository to notify you if a newer version of the utility is available, ensuring you are always running the latest code and features.</span>
